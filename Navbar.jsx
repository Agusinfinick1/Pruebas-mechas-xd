...
export default function Navbar({ cartCount, onCartClick, onNavigate, currentView }) {
  ...
  // Ocultar navbar dentro del panel admin (tiene su propio header)
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 flex items-center justify-between h-[72px]">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Acceso admin — texto discreto, solo desktop */}
            <button
              onClick={() => onNavigate("admin")}
              title="Acceso administración"
              className="hidden lg:flex items-center justify-center px-3 py-1.5 rounded-full text-[11px] font-body font-semibold text-ink/20 hover:text-ink/50 hover:bg-sand transition-all duration-300 tracking-wider uppercase"
        ...
        >
          <nav className="flex flex-col px-6 py-4 gap-2 bg-white border-t border-dust">
            ...
            {/* Admin en mobile — al final, muy sutil */}
            <button
              onClick={() => { setMobileOpen(false); onNavigate("admin") }}
              className="font-body text-xs text-ink/25 hover:text-ink/50 py-2 px-4 text-left transition-colors uppercase tracking-wider"
            >
              Acceso admin
            </button>
          </nav>
